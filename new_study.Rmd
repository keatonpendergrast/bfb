...
proptest <- pairwise.prop.test(prop$prop, prop$no)
proptest

proptrend <- prop.trend.test(prop$prop, prop$no)
proptrend
```
...
buprenorphine <- data_2summary2 %>%
  mutate(prop = BUPRENORPHINE*1000) %>%
  mutate(no = 1000) %>%
  select(prop, no)

proptest_bup <- pairwise.prop.test(buprenorphine$prop, buprenorphine$no)
proptest_bup

proptrend_bup <- prop.trend.test(buprenorphine$prop, buprenorphine$no)
proptrend_bup

CODEINE <- data_2summary2 %>%
  mutate(prop = CODEINE*1000) %>%
  mutate(no = 1000) %>%
  select(prop, no)

proptest_cod <- pairwise.prop.test(CODEINE$prop, CODEINE$no)
proptest_cod

proptrend_cod <- prop.trend.test(CODEINE$prop, CODEINE$no)
proptrend_cod

FENTANYL <- data_2summary2 %>%
  mutate(prop = FENTANYL*1000) %>%
  mutate(no = 1000) %>%
  select(prop, no)

proptest_fen <- pairwise.prop.test(FENTANYL$prop, FENTANYL$no)
proptest_fen

proptrend_fen <- prop.trend.test(FENTANYL$prop, FENTANYL$no)
proptrend_fen

HYDROMORPHONE <- data_2summary2 %>%
  mutate(prop = HYDROMORPHONE*1000) %>%
  mutate(no = 1000) %>%
  select(prop, no)

proptest_hydrmp <- pairwise.prop.test(HYDROMORPHONE$prop, HYDROMORPHONE$no)
proptest_hydrmp

proptrend_hydrmp <- prop.trend.test(HYDROMORPHONE$prop, HYDROMORPHONE$no)
proptrend_hydrmp
```
...
proptest2 <- pairwise.prop.test(prop2$prop, prop2$no)
proptest2

proptrend2 <- prop.trend.test(prop2$prop, prop2$no)
proptrend2
```
