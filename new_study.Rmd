...
new2 <- new %>%
  ...
  group_by(year) %>%
  summarise(sum = sum(PATWT, na.rm=T), avg_age = mean(AGE), sd_age = sem(AGE))
```
...
data_8summary <- rbind(close, close1, close2, close3, close4, close5, close6, close7, close8, close9, close10) %>%
  select(year, prop, MED, avg_age, sd_age)
...
# the opioid rate PER year amongst MENTAL HEALTH patients including AGE and MED
ggplot(data_8summary, aes(year, avg_age, colour = MED)) + 
  geom_line() +
  theme(legend.position = "none")

data_8summary2b <- recast(data_8summary, year + variable ~ MED, id.var = c("MED", "year", "prop", "sd_age"))

write.csv(data_8summary2b, file = "mental_opioid_rate_byage.csv")

data_8summary2c <- recast(data_8summary, year + variable ~ MED, id.var = c("MED", "year", "prop", "avg_age"))
...
ggplot(data_8b, aes(year, avg_age)) + 
  geom_point() +
  geom_line() +
  theme(legend.position = "none")

write.csv(data_8b, file = "mental_opioid_rate_byageonly.csv")
```
